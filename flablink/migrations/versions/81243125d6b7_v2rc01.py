...
def upgrade() -> None:
    ...
    sa.Column('updated_at', sa.TIMESTAMP(), nullable=False),
    sa.Column('uid', sa.Integer(), autoincrement=True, nullable=False),
    sa.Column('lims_keyword_uid', sa.Integer(), nullable=True),
    sa.Column('mapping', sa.String(length=50), nullable=True),
    sa.ForeignKeyConstraint(['lims_keyword_uid'], ['lims_keywords.uid'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('uid')
    )
    op.create_index(op.f('ix_lims_mappings_uid'), 'lims_mappings', ['uid'], unique=False)
